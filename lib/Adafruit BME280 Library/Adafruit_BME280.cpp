...
/*!
 *  @brief  Compensates the temperature data from the device
 *  @returns the compensated temperature value
 */
int32_t Adafruit_BME280::compTemperature(int32_t adc_T)
{
  int32_t var1, var2;
...
/*!
 *  @brief  Compensates the pressure data from the device
 *  @returns the compensated pressure value
 */
int32_t Adafruit_BME280::compPressure(int32_t adc_P)
{
...
/*!
 *  @brief  Compensates the humidity data from the device
 *  @returns the compensated humidity value
 */
int32_t Adafruit_BME280::compHumidity(int32_t adc_H)
{
...
//int32_t t_fine;
int32_t Adafruit_BME280::BME280_compensate_T_int32(int32_t adc_T)
{
  int32_t var1, var2, T;
  var1 = ((((adc_T >> 3) - ((int32_t)_bme280_calib.dig_T1 << 1))) * ((int32_t)_bme280_calib.dig_T2)) >> 11;
  var2 = (((((adc_T >> 4) - ((int32_t)_bme280_calib.dig_T1)) * ((adc_T >> 4) - ((int32_t)_bme280_calib.dig_T1)))
     >> 12) * ((int32_t)_bme280_calib.dig_T3)) >> 14;
  t_fine = var1 + var2;
  T = (t_fine * 5 + 128) >> 8;
  return T;
}
...
uint32_t Adafruit_BME280::BME280_compensate_P_int64(int32_t adc_P)
{
  int64_t var1, var2, p;
  var1 = ((int64_t)t_fine) - 128000;
  var2 = var1 * var1 * (int64_t)_bme280_calib.dig_P6;
...
uint32_t Adafruit_BME280::BME280_compensate_H_int32(int32_t adc_H)
{
  int32_t v_x1_u32r;
  ...
  v_x1_u32r = (((((adc_H << 14) - (((int32_t)_bme280_calib.dig_H4) << 20) -
                 ...
                 ((int32_t)16384)) >>
                15) *
               (((((((v_x1_u32r * ((int32_t)_bme280_calib.dig_H6)) >>
                     10) *
...
float Adafruit_BME280::readTemperature(void)
{
  ...
  return ((float)compTemperature(adc_T)) / 100;
}
...
float Adafruit_BME280::readHumidity(void)
{
  ...
  return ((float)compHumidity(adc_H)) / 1024;
}

/*!
 *   @brief  Burst read all sensor data at one time, and compensate data
 *   @param *pP pointer to pressure return value
 *   @param *pT pointer to temperature return value
 *   @param *pH pointer to humidity return value
 */
void Adafruit_BME280::readAllSensors(float_t *pP, float_t *pT, float_t *pH)
{
  uint8_t buffer[8];

  buffer[0] = BME280_REGISTER_PRESSUREDATA | (i2c_dev ? 0 : 1);
  i2c_dev->write_then_read(buffer, 1, buffer, 8);

  // Compensate temperature first to update t_fine.
  if (pT)
  {
    int32_t t = ((uint32_t)buffer[3]) << 16 | ((uint32_t)buffer[4]) << 8 | ((uint32_t)buffer[5]);
    *pT = ((float)compTemperature(t)) / 100;

    // Serial.printf("T: %i  %i\n", compTemperature(t), BME280_compensate_T_int32(t));
  }

  // Compensate pressure
  if (pP)
  {
    int32_t p = ((uint32_t)buffer[0]) << 16 | ((uint32_t)buffer[1]) << 8 | ((uint32_t)buffer[2]);
    *pP = (float)compPressure(p);

    // Serial.printf("P: %i  %i\n", compPressure(p), BME280_compensate_P_int64(p));
  }

  // Compensate humidity
  if (pH)
  {
    int32_t h = ((uint32_t)buffer[6]) << 8 | ((uint32_t)buffer[7]);
    *pH = ((float)compHumidity(h)) / 1024;

    // Serial.printf("H: %i  %i\n", compHumidity(h), BME280_compensate_H_int32(h));
  }
}
